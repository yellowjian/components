...
const RadioBox = forwardRef((props, ref) => {
  ...
  const themeStyle = themeConfig? {
    ':not(.disabled):hover': {
      ':before': {
        boxShadow: `1px 1px 0 ${getCSS(themeConfig.checkbox.hover.shadowColor)}, inset 0 0 3px ${getCSS(themeConfig.checkbox.hover.insetColor)}`
      }
    },
    ':before': {
      backgroundColor: getCSS(themeConfig.checkbox.backgroundColor),
      borderColor: getCSS(themeConfig.checkbox.borderColor),
      boxShadow: `1px 1px 0 ${getCSS(themeConfig.checkbox.shadowColor)}, inset 1px 1px 1px ${getCSS(themeConfig.checkbox.insetColor)}`
    },
    '&.selected': {
