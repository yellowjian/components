...
const Dropdown = forwardRef((props, ref) => {
  ...
  const getCurLabel = (selectedVals) => {
    const curOption = options.filter(item => item.value == selectedVals? selectedVals[0]: '')
    return curOption.length ? curOption[0].label: ''
  }
  const btnLabel = multiple ? `Selected (${selectedVals.length})` : getCurLabel(selectedVals) 
  return (
    <div className={dropdownClass} id={id} ref={dropdownRef}>
      <Button themeCss={{width: width}} label={btnLabel} icon='arrow_down' iconPosition='right' onClick={(e) => onButtonClick(e)}/>
      <Modal
        className={menuClass}
        isShow={isOpen}
